...
**Version:** 21 · **Last revised:** 2026-07-13

**Changelog**
- **v21 (2026-07-13)** — Weekly maintenance run. Job A: all 93 records refreshed (92 updated, Rob Garza *Dust Ups* still never-sold, left unchanged); collection value ≈ €2,237.45. Job B: all 66 pre-existing wishlist items re-scouted. Job C: Spotify sync added 22 new albums across kitchen dancing, Spring!, newish nowish, and heat it :: cool it (Cigarettes After Sex, Rhye, Tei Shi, Japanese Breakfast, St. Vincent, Spoon, Blue Hawaii, Thievery Corporation, Bishop Briggs, Elle King, Morcheeba, Jackson C. Frank, Tash Sultana, Portishead, Adele, Miike Snow, Ebo Taylor, Bonobo, Coldplay, David Bowie, Lykke Li, Röyksopp *Senior*); "Your Top Songs" playlist could not be located in Susan's library this run (likely not yet generated for the current year) so that source was skipped; quiet drive, classical (370 songs), heat it :: cool it (329 songs), and Rudy Van Gelder (758 songs) were only partially sampled given their size — a dedicated deeper pass is recommended as a follow-up. sync-state.json updated (165 → 187 entries). Job C2: Amazon cart checked (active + saved-for-later), no vinyl/music items found this run. Job E (new checks, all green): cache-bust check found no drift (style.css v25, app.js v34 consistent everywhere); all 5 audio-preview canaries still resolve correctly (Led Zeppelin *IV*, Air *Moon Safari*, Fleetwood Mac *Rumours* → "The Chain", Beethoven → Barenboim, The Scientist → Roots Radics); YouTube tier still not configured (unchanged); cover-art spot check (11 records + 14 wishlist items sampled) found zero broken images; full smoke-test parity 9/9 green. Job D: all 7 pages 200, auth gate correct (401 on unauthenticated /api/records and /api/save-cover; wishlist intentionally ungated, confirmed not a regression), delete round-trip clean, wishlist now 88 items (82 medians / 85 asks / 88 covers). Documentation reconciled: all six docs read in full; found and fixed one stale fact — the Identity section still said "92 records" (catalog has held 93 since v7).
