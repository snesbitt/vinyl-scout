...
**Version:** 15 · **Last revised:** 2026-07-11
...
**Status:** ✓ Built, deployed, and verified live against the full catalog (2026-07-11), out of the normal phase order, at Susan's direct request. Rebuilt same-day as a multi-provider lookup after Spotify's own preview restriction turned out to affect the entire catalog (see "Why multi-provider" below), then improved same-day again after diagnosing a real Deezer matching bug (see v15 changelog). **Verified result: 81/93 records (87%) have a real, bug-free playable preview** — see "Live verification sweep" below.
...
- **Remaining 12/93 `no_match`** — each checked directly against Deezer's catalog (not just through the function) and confirmed genuinely absent under any title: classical/orchestral (Maria Callas, Duke Ellington, Oscar Peterson, Herbert von Karajan, Vladimir Horowitz), niche reggae/remix/novelty (Errol Brown & The Revolutionaries, Rob Garza's *Dust Ups*, The Scientist), two "Various Artists" compilations (*The Blues Volume 2*, *Verve // Remixed* — Deezer has other, unrelated releases under similar titles, not Susan's specific pressing), The Swingle Singers' *Christmastime*, and The Cure's *Standing on a Beach* (confirmed absent from Deezer's full 74-album Cure catalog under any title, including the alternate "Staring at the Sea" title used in some regions).
- **0 bugs at either pass**: every `available:true` result was HTTP 200 with a non-empty `preview_url`; sampled preview URLs across both passes were independently fetched and confirmed to return real audio (`206 Partial Content`, `audio/mpeg`) — not dead links.
- **Bottom line:** 81 of 93 albums have a real, working, bug-free in-app preview. The remaining 12 don't have one because the specific recording doesn't exist under any name on Deezer (the only provider that has ever produced a playable preview for this catalog) — a genuine data-availability gap, verified by direct catalog inspection, not a software bug.
