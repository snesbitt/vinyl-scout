...
export default async (req) => {
  ...
  try {
    ...
    if (confirm !== 'DELETE_ALL_RECORDS_NOW') return new Response(JSON.stringify({ error: 'Bad confirm' }), { status: 400, headers: { 'Content-Type': 'application/json' } });
    const store = getStore('records');
    const { blobs } = await store.list();
    let count = 0;
    for (const blob of blobs) { await store.delete(blob.key); count++; }
